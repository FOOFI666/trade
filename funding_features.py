...
def add_funding_zscore(df: pd.DataFrame, window: int) -> pd.DataFrame:
    """Добавляет z-score для колонки funding_rate."""
...
def add_open_interest_change(df: pd.DataFrame, window: int, col_name: str) -> pd.DataFrame:
    """Расчёт относительного изменения OI за rolling-окно."""
